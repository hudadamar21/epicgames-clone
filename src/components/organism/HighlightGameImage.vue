...
<template>
  <div
    class="relative w-full h-full bg-center bg-cover rounded-xl"
  ...
  >
    ...
    ></div>
    <div
      class="absolute bottom-8 left-5 md:left-8 text-sm font-semibold md:font-bold w-[80%] md:w-[40%]"
    >
      ...
      <div class="items-center hidden h-12 gap-2 mt-3 md:flex">
        <BaseButton
          class="px-10 text-xs font-semibold text-black bg-white hover:text-black/80"
        >Buy Now</BaseButton>
        <BaseButton class="flex items-center justify-center gap-2 px-5 hover:bg-white/10">
          <AddIcon class="w-6 h-6" />
          <span class="font-bold text-xxs text-shadow w-30">Add To Wishlist</span>
        </BaseButton>
      </div>
    </div>
    <button
